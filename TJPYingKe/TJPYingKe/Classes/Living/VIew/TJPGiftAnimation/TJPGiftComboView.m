...
@implementation TJPGiftComboView
...
- (void) initialize{
    ...
    self.cacheNum = 0;
    //状态初始为闲置
    self.state = TJPGiftComboViewStateIdle;
}
...
- (void)setGiftModel:(TJPGiftModel *)giftModel {
...
- (void)performShowAnimation {
    //状态变为开始动画
    self.state = TJPGiftComboViewStateAnimating;
    //执行栈道动画
...
- (void)performGiftCountLabelAnimation {
    ...
    [self.giftCountLabel startAnimation:^{
        ...
        if (_cacheNum > 0) {
            ...
            //状态变为等待结束
            self.state = TJPGiftComboViewStateWaiting;
        }
    }];
}
...
- (void)perFormDismissAnimationi {
    //状态变为结束
    self.state = TJPGiftComboViewStateEnding;
    [UIView animateWithDuration:kAnimateDuration animations:^{
...
- (void)addOnceAnimationToCache {
    if (self.state == TJPGiftComboViewStateAnimating) { //正在执行动画的时候 缓存++
        _cacheNum += 1;
    }else if (self.state == TJPGiftComboViewStateWaiting) { //正在等待结束
        //取消之前3秒后要结束的方法 并执行label动画
        [NSObject cancelPreviousPerformRequestsWithTarget:self];
        [self performGiftCountLabelAnimation];
    }else if (self.state == TJPGiftComboViewStateIdle) { //闲置
//        [self performShowAnimation];
    }else {
